...
The tunnel may print a public hostname before DNS/edge propagation completes. MAR therefore performs a token-bound `/health/<token>` round trip through the public URL before returning `LINK_READY`.
