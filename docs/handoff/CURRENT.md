...
Immediate authorized implementation scope remains **Phase 0 + Phase 1 only**. Phase 0 is measured in `docs/research/034_HYBRID_SIMPLIFICATION_PHASE0_BASELINE.md` / `.json`: `.mar` = 16.882 GiB after prior cache cleanup; 114 live workspace paths remain; 104 BLOCKED workspaces are already physically terminated yet retain ~7.05 GiB. Phase 1 is now implemented as the scoped candidate `docs/implementation/035_HYBRID_PHASE1_CLEANUP_FIRST_RESOURCE.md`: disk-pressure admission attempts bounded safe reclamation, clears only positive-allowlist rebuildable cache when no MAR claims are active, invalidates stale disk telemetry and retries the same claim. BLOCKED workspaces remain untouched. Exact-candidate qualification after a scoped commit is still required before Phase 1 closes; checkpoint/rehydrate remains Phase 2. Existing activation-process-stop work is compatibility-only and remains outside the Phase 1 commit.
