# Current SoT — 2026-09-24

## Forward architecture — STABLE
...
**Trusted Owner Fast Path is the default normal-development path.** A Git project may be attached with explicit `network_allowed=true` so local host verification is usable immediately; omission remains fail-closed. Lightweight discovery now includes `project.context_batch`, which reuses existing find/search/read primitives in one round-trip and can optionally include bounded `git_status` so the common context+working-tree check stays one MCP call. Because ChatGPT may freeze an older input schema, `project.context_status` provides the same context+status composition using only pre-existing request fields and therefore remains callable without a connector schema refresh. `project.read/read_many` now return the exact full-file SHA-256 revision with bounded content, eliminating the extra host hash command before hash-guarded mutations. Measured on the MAR repo: `context_batch` median 31.35 ms versus 31.09 ms for equivalent separate local work while reducing MCP round-trips from 3 to 1. A direct heavy Project Brain wrapper was rejected after measuring 966.29 ms versus 33.05 ms for the separate pipeline. MAR exposes direct bounded project `action` operations for hash-guarded write/patch, local `mkdir` plus hash-guarded file `remove`/`rename` without requiring host-network authority, argv-only run or stop-on-failure `run_many`, Git branch/worktree helpers, stage/commit or one-round-trip `git_stage_commit`, and non-force push without creating a durable Goal/task/brain/verification lifecycle. The existing governed Goal path remains available when a task actually needs high-assurance isolation, exact-candidate verification, publication authorization, or recovery guarantees.

Architecture closure is finite and now complete. Publication/cancellation linearization is verified; private non-canonical staging, fresh exact-OID verification, OID handoff/import and expected-head CAS publication are verified/activated through Slice 057; fail-closed worker launch-environment defaults are verified/activated through Slice 058; the durable-state deletion boundary is verified/activated through Slice 059 at code-bearing revision `28fba4e2ff91c2d475930c75f0af172a10f6e7b9`. The bounded kernel blocker list is empty. ChatCode replacement direct-client proof has passed on the live Secure Tunnel path with no ChatCode process in the execution path.
...
- Trusted Owner Fast Path now has bounded local filesystem `mkdir`, exact-SHA file `remove`, and exact-SHA file `rename` without requiring host-network authority; focused and full release-style qualification PASS. Evidence: `docs/implementation/051_TRUSTED_OWNER_FAST_PATH_FILESYSTEM.md`.
- `project.context_batch` can optionally include bounded `git_status` in the same MCP response, removing one outer round-trip from the common context+working-tree discovery flow. Evidence: `docs/implementation/052_CONTEXT_BATCH_GIT_STATUS.md`.
- `project.read/read_many` now surface the exact full-file SHA-256 revision already computed from the bounded payload, so hash-guarded fast-path mutations no longer require a separate host `Get-FileHash` round-trip or network authority. Evidence: `docs/implementation/053_PROJECT_READ_SHA256.md`.
- `action.git_stage_commit` composes the existing bounded stage then commit primitives inside one MCP call, removing one outer round-trip without changing Git authority or introducing new service/kernel machinery. Evidence: `docs/implementation/054_GIT_STAGE_COMMIT_COMPOUND.md`.
- Representative fast-path benchmarking confirms that current direct-MAR productivity is dominated by outer ChatWeb/MCP round-trip count rather than local deterministic execution. The current live ChatCode same-workload leg is `NOT_RUN` because ChatCode rejected admission at its 3/3 AI-connection limit; no global MAR-vs-ChatCode speed winner is claimed. Evidence: `docs/research/MAR_CHATCODE_FAST_PATH_BENCHMARK_2026-09-23.md`.
- `project.context_status` is the bounded frozen-schema-safe follow-up to Slice 052: it composes context batch + Git status using only request fields already present in older `project` schemas. Exact revision `f5ada0d6e29fbdc3fc81e431349150cc038b4233` is verified/activated; the current stale-schema ChatGPT conversation called `context_status` successfully without reconnect in ~2.4 s and received both context batch + Git status. Evidence: `docs/implementation/056_FROZEN_SCHEMA_CONTEXT_STATUS.md`.
- Governed integration Slice 057 is verified/activated at exact revision `2b95bfd61ea1f198f7eed2253a69c49d9d44543a`: changed candidates are anchored under `refs/mar/integrations/...` before dispatch; fresh result identity is rebound through exact version/evidence/final OID; the existing expected-head canonical `update-ref` CAS is retained; private anchors are cleaned on cancellation/block/success and retained while post-CAS checkout sync remains recoverable. Full exact-HEAD test/vet/build/diff-check PASS and live runtime is `HEALTHY / ALIGNED / trusted_for_release=true`. Evidence: `docs/implementation/057_PRIVATE_INTEGRATION_CANDIDATE_ANCHOR.md`.
- Governed worker launch Slice 058 is verified/activated at exact revision `2dee9f1e2129cce0c4a49dde10c18a1801c144b7`: `ProcessRunner` rejects nil environment instead of inheriting `os.Environ()`, production worker launch uses a bounded Windows/PATH whitelist plus only the explicitly named Provider credential and explicit extras, Provider/Harness/WebBrain self-hosting E2E PASS, full exact-HEAD test/vet/build/diff-check PASS, and live runtime is `HEALTHY / ALIGNED / trusted_for_release=true`. Evidence: `docs/implementation/058_FAIL_CLOSED_WORKER_LAUNCH_ENVIRONMENT.md`.
- Slice 059 is verified/activated at exact code-bearing revision `28fba4e2ff91c2d475930c75f0af172a10f6e7b9`: only durably `REHYDRATED` checkpoint refs are releasable; COMPACTED/REHYDRATING refs remain recovery authority; exact-OID mismatch fails closed; release is crash-idempotent and leaves task/attempt/result/evidence/checkpoint rows intact. Full exact-HEAD test/vet/build/diff-check PASS. Live acceptance released REHYDRATED active refs 16 -> 0, persisted 16 release receipts, preserved COMPACTED active refs at 134, and reduced physical `D:\MAR` checkpoint refs 132 -> 116 while runtime remained `HEALTHY / ALIGNED / trusted_for_release=true`. Evidence: `docs/implementation/059_DURABLE_STATE_DELETION_BOUNDARY.md`.
- Slice 060 is verified/activated at exact code-bearing revision `c7f7e2922abd12af3080d9146d30954f602d5c80`. The bounded MCP tool observer exposes only tool/operation/project/start/complete/duration/outcome metadata, retains no arguments/output/secrets, uses a bounded in-memory 256-event buffer with dropped count, isolates observer panics, and keeps fast-path work non-durable. Independent audit found and repaired one tool-level error-classification defect (`IsError=true` had initially appeared as `outcome=ok`); regression coverage now locks the corrected semantics. Full exact-HEAD test/vet/build/diff-check PASS. Live acceptance from the already-connected ChatGPT path showed AutoVideoPipeline `project.context` start/complete metadata in `/api/runtime` while active durable AutoVideoPipeline task count remained 0. React Live Operations now projects fast-path activity plus a separate authoritative token-throughput heartbeat. Evidence: `docs/implementation/060_FAST_PATH_LIVE_OBSERVABILITY.md`.

Next MAR implementation work: **STOP architecture/kernel/fast-path expansion by default.** Use this stable line for ordinary projects. Do not open another slice unless representative failing evidence demonstrates a new concrete deficit under the accepted stability policy. The unavailable ChatCode same-workload benchmark leg may be rerun when a connection slot exists, but it is optional measurement and does not reopen or block stability.
...
**Current post-V1.3 capability line:** the accumulated capability line is now release-qualified and activated at exact revision `2782f971d7ce2e885078c4d5b7cbc33962471cec`. Exact-HEAD release gate PASS (`go test -p 1 -count=1 -timeout 300s ./...`, `go vet -p 1 ./...`, `go build -p 1 ./...`, `git diff --check`), local/remote Git identity converged, and the live runtime reports `HEALTHY / ALIGNED / trusted_for_release=true` with the OpenAI Secure Tunnel connected/ready/healthy. Slices E/F remain closed. The next bounded product action is representative MAR-vs-ChatCode workflow benchmarking; open only measured capability/performance gaps and do not add another optimization slice by default.
