...
Next MAR implementation work: **STOP architecture/kernel/fast-path expansion by default.** Use this stable line for ordinary projects. Open a new bounded slice only when representative failing evidence demonstrates a concrete deficit under the accepted stability policy. The unavailable ChatCode same-workload benchmark leg may be rerun when a connection slot exists, but it is optional measurement and does not reopen or block stability.
