...
function WorkspacesPage({projects,reload,workspace,setWorkspace}:any){const [root,setRoot]=useState(''),[id,setId]=useState(''),[busy,setBusy]=useState(false),[pickerBusy,setPickerBusy]=useState(false),[error,setError]=useState('');async function chooseFolder(){setPickerBusy(true);setError('');try{const r:any=await pickProject();if(r.path)setRoot(r.path)}catch(e:any){setError(e?.message||'Không mở được trình chọn thư mục.')}finally{setPickerBusy(false)}}async function register(){if(!root)return;setBusy(true);setError('');try{const created:any=await addProject(root,id);setRoot('');setId('');await reload();if(created?.project_id)setWorkspace(created.project_id)}catch(e:any){setError(e?.message||'Không thể thêm workspace.')}finally{setBusy(false)}}return <><div className="page-header"><div><h1>Workspaces</h1><p>Đăng ký Git repository local và quyền mặc định.</p></div>{workspace&&<button className="scope-button" onClick={()=>setWorkspace('')}><XCircle size={15}/>Bỏ lọc: {workspace}</button>}</div><div className="workspace-grid"><section className="panel add-workspace"><div className="panel-heading"><div className="panel-title"><Plus size={20}/><h2>Thêm workspace</h2></div></div><label>Thư mục Git repository<div className="picker"><button disabled={pickerBusy} onClick={chooseFolder}>{pickerBusy?<LoaderCircle className="spin" size={16}/>:<FolderGit2 size={16}/>}Chọn thư mục</button><input value={root} onChange={e=>setRoot(e.target.value)} placeholder="D:\\Projects\\MyApp"/></div></label><label>Project ID <span className="muted">(tùy chọn)</span><input value={id} onChange={e=>setId(e.target.value)} placeholder="Tự tạo nếu để trống"/></label>{error&&<div className="form-error">{error}</div>}<button className="primary-button" disabled={!root||busy} onClick={register}>{busy?<LoaderCircle className="spin" size={16}/>:<Plus size={16}/>}Thêm workspace</button></section><section className="panel workspace-list-panel"><div className="panel-heading"><div className="panel-title"><FolderGit2 size={20}/><h2>Đã đăng ký</h2></div><IconButton icon={RefreshCw} label="Làm mới" onClick={reload}/></div><div className="workspace-cards">{projects.map((p:any)=><WorkspaceCard key={p.id} p={p} reload={reload} selected={workspace===p.id} onSelect={()=>setWorkspace(p.id)}/>)}</div></section></div></>}
function WorkspaceCard({p,reload,selected,onSelect}:any){const [file,setFile]=useState(!!p.policy?.local_file_write),[git,setGit]=useState(!!p.policy?.local_git_write);useEffect(()=>{setFile(!!p.policy?.local_file_write);setGit(!!p.policy?.local_git_write)},[p]);return <div className={`workspace-card ${selected?'selected':''}`}><div><div className="workspace-title"><strong>{p.id}</strong><span className={`support-chip ${p.supported?'ok':'bad'}`}>{p.supported?'Sẵn sàng':'Chưa hỗ trợ'}</span>{selected&&<span className="support-chip selected-chip">Đang chọn</span>}</div><code>{p.root}</code><small>{p.head?`HEAD ${String(p.head).slice(0,12)}`:(p.support_reason||p.head_error||'HEAD unavailable')}</small></div><div className="policy-checks"><label><input type="checkbox" checked={file} onChange={e=>setFile(e.target.checked)}/>Sửa file local</label><label><input type="checkbox" checked={git} onChange={e=>setGit(e.target.checked)}/>Local Git</label><span>Network / push / deploy: off</span><button className={`workspace-select-button ${selected?'selected':''}`} onClick={onSelect} disabled={selected}>{selected?'Đang chọn':'Chọn'}</button><button onClick={async()=>{await updateProjectPolicy(p.id,file,git);await reload()}}>Lưu quyền</button></div></div>}
