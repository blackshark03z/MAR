...
import {
  addProject, cancelTask, connectionAction, createTask, feedbackTask, fmtDuration,
  fmtNumber, fmtTime, getProjects, getRuntime, getTaskFeedback, getTaskInspect,
  getTaskResult, getTasks, getTaskStatus, getUsage, inputTask, pickProject,
...
function primaryConnections(runtime: any) {
  ...
  const gpt = routeReady(tunnel) ? tunnel : (routeReady(fallback) ? fallback : (tunnel || fallback))
...
function Shell({ view, setView, runtime, workspace, setWorkspace, projects, children }: any) {
  return <div className="app-shell">
    <header className="topbar">
      ...
      <div className="topbar-center"><label>Workspace<select value={workspace} onChange={e=>setWorkspace(e.target.value)}><option value="">Tất cả workspaces</option>{projects.map((p:any)=><option key={p.id} value={p.id}>{p.id}</option>)}</select></label></div>
      <div className="runtime-pills">
        <span className="runtime-pill live"><span className="live-dot"/>Telemetry: live</span>
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-health"><span className="live-dot"/><div><strong>Hệ thống hoạt động</strong><small>Mọi thứ đang ổn định</small></div></div>
...
function Overview({ runtime, tasks, usage, setView, workspace }: any) {
  const scoped = workspace ? tasks.filter((t:any)=>t.project_id===workspace) : tasks
  const active = scoped.filter(isTaskActive)
  const blocked = scoped.filter((t:any)=>['BLOCKED','FAILED','INPUT_REQUIRED'].includes(String(t.state).toUpperCase()))
  const health = systemHealth(runtime)
...
function TasksPage({ tasks, projects, workspace, reloadTasks }: any) {
  const scoped = workspace ? tasks.filter((t:any)=>t.project_id===workspace) : tasks
  const [filter,setFilter]=useState('all'), [query,setQuery]=useState(''), [selected,setSelected]=useState<string>('')
  useEffect(()=>{if(!selected && scoped.length)setSelected((scoped.find((t:any)=>t.needs_attention)||scoped[0]).id);if(selected&&!scoped.some((t:any)=>t.id===selected))setSelected('')},[tasks,workspace])
  const filtered = scoped.filter((t:any)=>{const state=String(t.state||'').toUpperCase();const status=filter==='all'||(filter==='active'&&(isTaskActive(t)||state==='INPUT_REQUIRED'))||(filter==='blocked'&&['BLOCKED','FAILED','INPUT_REQUIRED'].includes(state))||(filter==='complete'&&state==='COMPLETE');const text=`${t.goal||''} ${t.id||''} ${t.project_id||''}`.toLowerCase();return status&&(!query||text.includes(query.toLowerCase()))})
  return <><div className="page-header"><div><h1>Tasks</h1><p>Theo dõi và quản lý các tác vụ AI trên toàn bộ workspaces.</p></div></div><div className="task-toolbar"><select value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">Tất cả · {scoped.length}</option><option value="active">Đang chạy</option><option value="blocked">Blocked</option><option value="complete">Hoàn tất</option></select><div className="search-box"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Tìm task theo tiêu đề, id…"/></div></div><div className="task-three-col"><section className="panel task-list-panel"><div className="panel-heading"><div className="panel-title"><ListTodo size={19}/><h2>Danh sách task ({filtered.length})</h2></div><IconButton icon={RefreshCw} label="Làm mới" onClick={reloadTasks}/></div><div className="task-list">{filtered.length?filtered.map((t:any)=><button className={`task-item ${selected===t.id?'selected':''}`} key={t.id} onClick={()=>setSelected(t.id)}><div className="task-item-top"><strong>{t.goal||'Untitled task'}</strong><StatusBadge state={t.state}/></div><div className="task-item-meta"><span>{t.project_id}</span><span>{t.live_usage?.tokens_available?`~${fmtNumber(t.live_usage.total_tokens)} tokens`:fmtNumber(t.usage?.model_total_tokens||0)+' tokens'}</span><span>{fmtTime(t.updated_at)}</span></div></button>):<EmptyState icon={ListTodo} title="Không có task phù hợp"/>}</div></section><TaskDetail id={selected} reloadTasks={reloadTasks}/><CreateTaskPanel projects={projects} preferredWorkspace={workspace} onCreated={reloadTasks}/></div></>
}
function TaskDetail({ id, reloadTasks }: any) {
...
function WorkspacesPage({projects,reload}:any){const [root,setRoot]=useState(''),[id,setId]=useState(''),[busy,setBusy]=useState(false);return <><div className="page-header"><div><h1>Workspaces</h1><p>Đăng ký Git repository local và quyền mặc định.</p></div></div><div className="workspace-grid"><section className="panel add-workspace"><div className="panel-heading"><div className="panel-title"><Plus size={20}/><h2>Thêm workspace</h2></div></div><label>Thư mục Git repository<div className="picker"><button onClick={async()=>{const r:any=await pickProject();if(r.path)setRoot(r.path)}}><FolderGit2 size={16}/>Chọn thư mục</button><input value={root} onChange={e=>setRoot(e.target.value)} placeholder="D:\\Projects\\MyApp"/></div></label><label>Project ID <span className="muted">(tùy chọn)</span><input value={id} onChange={e=>setId(e.target.value)} placeholder="Tự tạo nếu để trống"/></label><button className="primary-button" disabled={!root||busy} onClick={async()=>{setBusy(true);try{await addProject(root,id);setRoot('');setId('');await reload()}finally{setBusy(false)}}}><Plus size={16}/>Thêm workspace</button></section><section className="panel workspace-list-panel"><div className="panel-heading"><div className="panel-title"><FolderGit2 size={20}/><h2>Đã đăng ký</h2></div><IconButton icon={RefreshCw} label="Làm mới" onClick={reload}/></div><div className="workspace-cards">{projects.map((p:any)=><WorkspaceCard key={p.id} p={p} reload={reload}/>)}</div></section></div></>}
function WorkspaceCard({p,reload}:any){const [file,setFile]=useState(!!p.policy?.local_file_write),[git,setGit]=useState(!!p.policy?.local_git_write);useEffect(()=>{setFile(!!p.policy?.local_file_write);setGit(!!p.policy?.local_git_write)},[p]);return <div className="workspace-card"><div><div className="workspace-title"><strong>{p.id}</strong><span className={`support-chip ${p.supported?'ok':'bad'}`}>{p.supported?'Sẵn sàng':'Chưa hỗ trợ'}</span></div><code>{p.root}</code><small>{p.head?`HEAD ${String(p.head).slice(0,12)}`:(p.support_reason||p.head_error||'HEAD unavailable')}</small></div><div className="policy-checks"><label><input type="checkbox" checked={file} onChange={e=>setFile(e.target.checked)}/>Sửa file local</label><label><input type="checkbox" checked={git} onChange={e=>setGit(e.target.checked)}/>Local Git</label><span>Network / push / deploy: off</span><button onClick={async()=>{await updateProjectPolicy(p.id,file,git);await reload()}}>Lưu quyền</button></div></div>}
...
function ConnectionCard({c,reload}:any){const provider=connectionProvider(c),ready=routeReady(c),isTunnel=c.id==='openai-tunnel';const [open,setOpen]=useState(false);async function act(action:string){let path='';if(isTunnel)path=`/api/connections/openai-tunnel/${action}`;else if(action==='start')path='/api/connections/web-bridge/start';else if(action==='restart')path='/api/connections/web-bridge/restart';else if(action==='stop')path='/api/connections/web-bridge/stop';else path=`/api/connections/${encodeURIComponent(c.id)}/diagnose`;await connectionAction(path);await reload()}return <section className={`panel connection-card ${provider.toLowerCase()}`}><div className="connection-head"><div className={`provider-brand ${provider==='Claude'?'claude':''}`}><Bot size={24}/></div><div><h2>{c.name||provider}</h2><p>{c.transport||c.summary||''}</p></div><span className={`connection-chip ${ready?'connected':'idle'}`}><span className="live-dot"/>{ready?'Sẵn sàng':(c.status||'Idle')}</span></div><div className="connection-state"><Metric label="Configuration" value={c.configured===false?'Needs setup':'Configured'}/><Metric label="Transport" value={ready?'Ready':(c.status||'—')}/><Metric label="Sessions" value={c.active_sessions_available?fmtNumber(c.active_sessions||0):'—'}/></div>{c.identifier&&<div className="copy-row"><code>{c.identifier}</code><IconButton icon={Copy} label="Sao chép" onClick={()=>navigator.clipboard.writeText(c.identifier)}/></div>}<p className="connection-summary">{c.summary||c.next_action||'Không có ghi chú.'}</p><div className="connection-actions"><button className="primary-button" onClick={()=>act(c.running?'restart':'start')}>{c.running?<RotateCcw size={16}/>:<Play size={16}/>} {c.running?'Khởi động lại':'Kết nối'}</button>{c.running&&<button onClick={()=>act('stop')}><StopCircle size={16}/>Tạm dừng</button>}<button onClick={()=>act('diagnose')}><TerminalSquare size={16}/>Chẩn đoán</button><button onClick={()=>setOpen(!open)}><Settings size={16}/>Chi tiết</button></div>{open&&<pre className="connection-raw">{JSON.stringify(c,null,2)}</pre>}</section>}
...
export default function App(){
  ...
  useEffect(()=>{Promise.all([reloadRuntime(),reloadProjects(),reloadTasks(),reloadUsage()]);const timer=setInterval(async()=>{await Promise.allSettled([reloadRuntime(),reloadTasks()])},2000);const usageTimer=setInterval(reloadUsage,30000);return()=>{clearInterval(timer);clearInterval(usageTimer)}},[])
  ...
  else if(view==='overview')content=<Overview runtime={runtime} tasks={tasks} usage={usage} setView={setView} workspace={workspace}/>
  else if(view==='tasks')content=<TasksPage tasks={tasks} projects={projects} workspace={workspace} reloadTasks={reloadTasks}/>
  else if(view==='workspaces')content=<WorkspacesPage projects={projects} reload={reloadProjects}/>
