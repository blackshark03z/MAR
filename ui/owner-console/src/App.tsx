...
function WorkspaceCard({p,reload,selected,onSelect}:any){const [file,setFile]=useState(!!p.policy?.local_file_write),[git,setGit]=useState(!!p.policy?.local_git_write),[network,setNetwork]=useState(!!p.policy?.network_allowed),[push,setPush]=useState(!!p.policy?.remote_git_write),[deploy,setDeploy]=useState(!!p.policy?.deploy_allowed),[saving,setSaving]=useState(false),[message,setMessage]=useState(''),[error,setError]=useState('');useEffect(()=>{setFile(!!p.policy?.local_file_write);setGit(!!p.policy?.local_git_write);setNetwork(!!p.policy?.network_allowed);setPush(!!p.policy?.remote_git_write);setDeploy(!!p.policy?.deploy_allowed)},[p]);return <div className={`workspace-card ${selected?'selected':''}`}><div><div className="workspace-title"><strong>{p.id}</strong><span className={`support-chip ${p.supported?'ok':'bad'}`}>{p.supported?'Sẵn sàng':'Chưa hỗ trợ'}</span>{selected&&<span className="support-chip selected-chip">Đang chọn</span>}</div><code>{p.root}</code><small>{p.head?`HEAD ${String(p.head).slice(0,12)}`:(p.support_reason||p.head_error||'HEAD unavailable')}</small></div><div className="policy-checks"><label><input type="checkbox" checked={file} onChange={e=>setFile(e.target.checked)}/>Sửa file local</label><label><input type="checkbox" checked={git} onChange={e=>setGit(e.target.checked)}/>Local Git</label><label><input type="checkbox" checked={network} onChange={e=>setNetwork(e.target.checked)}/>Network: {network?'ON':'OFF'}</label><label><input type="checkbox" checked={push} onChange={e=>setPush(e.target.checked)}/>Push: {push?'ON':'OFF'}</label><label><input type="checkbox" checked={deploy} onChange={e=>setDeploy(e.target.checked)}/>Deploy: {deploy?'ON':'OFF'}</label><button className={`workspace-select-button ${selected?'selected':''}`} onClick={onSelect} disabled={selected}>{selected?'Đang chọn':'Chọn'}</button><button disabled={saving} onClick={async()=>{setSaving(true);setMessage('');setError('');try{await updateProjectPolicy(p.id,file,git,network,push,deploy);await reload();setMessage('Đã lưu quyền')}catch(e:any){setError(e?.message||'Không thể lưu quyền')}finally{setSaving(false)}}}>{saving?<><LoaderCircle className="spin" size={15}/>Đang lưu...</>:'Lưu quyền'}</button>{message&&<div className="action-feedback success"><CheckCircle2 size={15}/>{message}</div>}{error&&<div className="action-feedback error"><AlertTriangle size={15}/>{error}</div>}</div></div>}
...
export default function App(){
  ...
  useEffect(()=>{Promise.all([reloadRuntime(),reloadProjects(),reloadTasks(),reloadUsage()]);const timer=setInterval(async()=>{await Promise.allSettled([reloadRuntime(),reloadTasks()])},1000);const usageTimer=setInterval(reloadUsage,30000);const onHashChange=()=>{const next=(window.location.hash.replace('#/','')||'live') as View;if(views.some(v=>v.id===next))setViewState(next)};window.addEventListener('hashchange',onHashChange);return()=>{clearInterval(timer);clearInterval(usageTimer);window.removeEventListener('hashchange',onHashChange)}},[])
  ...
  useEffect(()=>{const tick=()=>{const a=aggregateRef.current;if(!a.tokenTasks){setTokenSamples(prev=>prev.length?[]:prev);return}setTokenSamples(prev=>[...prev,{at:Date.now(),input:a.input,output:a.output}].slice(-40))};tick();const timer=setInterval(tick,1000);return()=>clearInterval(timer)},[])
