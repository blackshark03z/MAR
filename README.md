...
`project` is the bounded discovery/attach surface. Its lightweight `context_batch` combines bounded find/search/snippet reads into one round-trip without invoking the heavier Project Brain scan; refreshed clients may request bounded `git_status` in that same response, while frozen-schema clients can use `operation=context_status` to get the same context+status composition using only pre-existing request fields. `read` and `read_many` return the exact full-file SHA-256 revision alongside bounded content, so the result can flow directly into hash-guarded fast-path mutations without a separate host hash command. Git attach stays fail-closed by default; when trusted-owner host verification is needed, the client can set `network_allowed=true` in that same attach call. `action` is the Trusted Owner Fast Path for ordinary development: hash-guarded create/replace and exact-hash patch, bounded local filesystem `mkdir` / hash-guarded `remove` / hash-guarded `rename` without host-network authority, argv-only bounded run or stop-on-failure `run_many`, Git branch/worktree helpers, stage/commit or one-round-trip `git_stage_commit`, and non-force push gated by project policy, with no durable Goal/task/brain lifecycle. `submit`, `task`, and `control` retain the governed high-assurance workflow; `brain_turn` and `brain_respond` remain its external-cognition relay.
