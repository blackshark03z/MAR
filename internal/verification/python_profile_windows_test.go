...
func TestResolveExecutionProfileAddsExistingPythonSelfTest(t *testing.T) {
	...
	resolved, err := resolveExecutionProfile(base, root)
	if err != nil {
		t.Fatal(err)
	}
	if len(resolved.Commands) != 2 || len(resolved.Commands[0].Args) != 1 || resolved.Commands[0].Args[0] != "scripts/self_test.py" {
		t.Fatalf("self-test was not prepended: %+v", resolved.Commands)
	}
	if len(base.Commands) != 1 {
		t.Fatal("declared profile was mutated")
	}
}
...
func TestResolveExecutionProfileKeepsFallbackWhenSelfTestAbsent(t *testing.T) {
	...
	resolved, err := resolveExecutionProfile(base, t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	if len(resolved.Commands) != 1 || resolved.Commands[0].Args[1] != "unittest" {
		t.Fatalf("fallback profile changed unexpectedly: %+v", resolved.Commands)
	}
}
